...
| `common/10-workflow.md` | 通用开发工作流与需求读取 | 任务分级、生效规则告知、查证收口、需求文档读取、需求与接口优先级、Git 操作规范 |
| `common/20-no-hallucination.md` | 通用禁止发散与最小侵入 | 禁止脑补、禁止静默发散、禁止顺手优化、精准替换 |
| `common/30-formatting.md` | 通用格式化规范 | Codex 修改代码后的格式化与轻量验证要求 |
| `common/40-comments.md` | 通用代码清理与注释 | 代码清理、注释内容边界、文件头注释 |
...
| `frontend/40-function-hook-reactive.md` | 前端 Hook 与响应式 | Vue Hook 抽离、computed 使用 |
...
1. **先定语言层归属**：项目存在 `package.json` / `.ts` 源码 → `lang/js-ts.md`；存在 `pom.xml` / `build.gradle` → `lang/java.md`；禁止默认「后端 = Java」「JS = 前端」。
2. **再看改动产物**：`.vue`、组件、Hook、SCSS、路由、i18n、Icon 属于前端；Controller、Service、DTO、Entity、Repository、SQL、Migration、队列、缓存、鉴权中间件属于后端；Git、需求读取、禁止脑补、注释、自检、终端命令属于通用。
3. **再看规则约束对象**：约束页面展示、组件写法和样式还原的是前端；约束接口实现、数据落库和服务端校验的是后端；约束 AI 开发前后如何确认和交付的是通用。
4. **最后看验证入口**：浏览器、Vite、SFC、样式检查偏前端；接口测试、数据库迁移、服务日志、鉴权链路偏后端；契约测试、跨端端到端验证、发布与回滚、`git diff --check`、规则同步和自检脚本偏通用。
5. **混合规则不强行塞单类**：接口字段不能脑补、跨接口字段归属、联调缺口反馈这类协作契约优先放 `common/`；必要时再在 `frontend/` 或 `backend/` 补专属细则。
...
4. **规则质量自审**：预览前提炼不变量和约束对象，检查未来绕过、合法场景误伤、事实失效、规则冲突和触发可见性；任一项未通过时先重写。
5. **预览确认**：写入前给用户展示规则内容、质量审计摘要、目标文件和目标章节；用户只负责确认意图和范围，不负责替 AI 发现规则设计缺陷。
6. **维护清单**：新增规则文件时，必须同步更新 `rules/rules-manifest.json` 和本文件索引；只补充已有文件时通常不需要更新 manifest。
7. **同步验证**：写入后执行 `python3 sync-workflows.py --no-git`；同步脚本会自动执行同步前源结构自检和同步后全量自检，确认拼接后的全局规则已同步到目标工具。
...
- 新规则是否基于稳定不变量和约束对象，而不是只覆盖当前案例或实现方式。
- 新规则是否通过未来绕过、合法场景误伤、事实失效和触发可见性审计。
- 新规则是否包含明确的禁止项和判断标准。
