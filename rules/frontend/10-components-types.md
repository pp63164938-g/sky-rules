...
- **条件判断必须先定语义方向再选函数**：编写 `if`、`filter`、`computed`、模板条件、接口参数过滤等条件判断前，必须先用自然语言确认当前条件要表达的业务语义，例如“为空则返回”“只保留有值项”“允许操作”“禁用状态”“命中目标类型”。然后选择同方向的函数或变量表达该语义，禁止先写反向函数再取反。
- **成对 / 互补判断函数必须按条件语义选择，不得先写反再取反**：当项目或公共库同时存在 `isEmpty` / `isNotEmpty`、`isNil` / `isNotNil`、`hasXxx` / `noXxx`、`canXxx` / `cannotXxx`、`isEnabled` / `isDisabled`、`isXxx` / `isNotXxx` 这类正反语义函数或变量时，必须选择与业务语义同方向的表达。
- **过滤和保留类逻辑必须用目标结果方向表达**：例如数组过滤“保留有效项”时，条件语义是“有值”，应写 `filter(item => isNotEmpty(item))`；禁止写 `filter(item => !isEmpty(item))`。如果过滤“剔除无效项”，也应先判断最终表达是否更适合写成正向保留条件。
- **复合条件同样禁止反向函数取反**：在 `if`、`computed`、`return`、模板表达式、`&&` / `||` 复合条件中，如果条件语义是“任一为空则返回”，应写 `isEmpty(a) || isEmpty(b)`；禁止写成 `!isNotEmpty(a) || !isNotEmpty(b)`。如果条件语义是“全部有值才展示”，应写 `isNotEmpty(a) && isNotEmpty(b)`；禁止写成 `!isEmpty(a) && !isEmpty(b)`。
- **数组长度判断与函数方向规则分层处理**：当条件语义是“数组未初始化或无元素则返回”，且后续代码需要 TypeScript 明确缩窄为数组时，可以使用 `!xxxList?.length` 这类数组长度判断；这属于结构 / 类型缩窄判断，不等同于通用空值函数。但进入数组元素过滤后，仍必须按目标结果方向选择判断函数。
- **缺少同方向函数时才允许取反**：只有项目或公共库没有同方向函数，或同方向函数语义与当前业务不一致时，才允许使用取反表达；如果取反条件不直观，必须在代码附近说明原因。
- **交付前必须搜索本次改动中的反向判断**：只要本次引入或修改了成对判断函数，交付前必须在本次修改文件中搜索 `!isEmpty`、`!isNotEmpty`、`!isXxx`、`!isNotXxx` 等反向写法。若存在同语义的反向函数，必须改为同方向函数；若确实没有反向函数或语义不等价，必须在代码附近说明原因。
...
```ts
// ✅ 正确 - 数组长度判断用于空数组返回和 TypeScript 缩窄
if (!xxxList?.length) return '-'

// ✅ 正确 - 目标是保留有值项，使用同方向函数
const validNameList = xxxList.filter(name => isNotEmpty(name))

// ❌ 禁止 - 目标是保留有值项，却用空值函数取反
const validNameList = xxxList.filter(name => !isEmpty(name))
```

```ts
// ❌ 禁止 - 条件语义是“为空则返回”，却使用“有值函数 + 取反”
if (!isNotEmpty(xxxValue) || !isNotEmpty(yyyValue)) {
    return
}

// ✅ 正确 - 条件语义和函数方向一致
if (isEmpty(xxxValue) || isEmpty(yyyValue)) {
    return
}
```

```ts
// ❌ 禁止 - 条件语义是“有值才继续”，却使用“为空函数 + 取反”
if (!isEmpty(xxxValue) && !isEmpty(yyyValue)) {
    submit()
}
...
**核心原则**：组件化依据是稳定、可独立命名的视图职责，而不是代码行数或是否已经复制整段模板。同一视图能力的数据准备、展示规范、交互状态和结构样式分散在父组件与多个场景组件中，也属于职责重复。

**必须组件化的边界**：

一个区域具有可独立命名的视图职责，且命中以下任一条件时，应拆成页面或模块业务组件：

- 被两个及以上页面、场景分支或调用方使用。
- 自身拥有展开、收起、选择、切换等局部交互状态。
- 自身统一处理换行、截断、空态、占位、展示格式等视图规范。
- 拥有独立 DOM 结构、样式、测试标识或可访问性交互。
- 同一职责被拆散：调用方各自整理展示数据，父组件再负责统一渲染和交互。
- 父组件只需要提供输入和接收事件，不需要了解该区域的内部状态。

**职责归属**：

- 业务调用方负责选择接口字段、判断业务场景、权限和业务枚举。
- 业务组件负责展示所需的数据规范化、空态、结构、样式和局部交互状态。
- 通用组件不得接收完整页面 DTO 后自行判断业务类型，应使用最小 props、events 或 slots。
- 页面内复用先放页面 `components/`；跨页面稳定复用后再提升到模块目录；跨模块稳定复用后才进入公共组件。

**不应拆成组件的场景**：

- 单次使用的简单文本、普通字段或无状态展示。
- 只有纯数据转换且不产生视图结构，应使用普通函数。
- 只有响应式流程或副作用但不负责 DOM，应按 Hook 规范判断。
- 拆分后需要镜像父组件大量私有状态或透传大量无关参数。
- 只是为了减少文件行数或提前假设未来可能复用。

**开发前检查**：

命中组件化信号时，开发前必须说明组件职责、输入输出、放置层级和不纳入组件的业务逻辑。局部修改若暴露出已有职责分散问题，应明确给出拆分判断；拆分会扩大当前任务范围时，先预览影响，不得静默重构。
