...
- **常量抽离前必须先判断它是稳定标识还是待联调预留**：常量适用于已确认的前端场景标识、跨页 bringType、稳定业务枚举、复用配置 key 等；不适用于包装未确认的接口字段、参数名、响应字段或数据源。若后端未提供字段，应按静态开发 / 联调规则使用 `TODO待联调_` / `TODO无此联调字段_` 显性预留，而不是抽成普通常量来隐藏不确定性。只有该值本身已确认且多处复用，或能形成稳定前端协议时，才允许抽常量，并必须用 JSDoc 写明来源、用途和是否不对应后端字段。
