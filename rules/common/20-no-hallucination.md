...
10. **待确认内容不得默认抽常量**：接口字段、查询参数、提交字段、响应字段、下拉数据源、枚举真实值等未确认内容，禁止为了“统一引用”或“方便后续替换”默认抽成普通常量。常量只适合承载已确认且稳定的前端场景标识、跨文件复用标识、真实业务枚举或项目约定值；如果不确定内容本质是后端契约缺口，应使用 `TODO待联调_用途描述` / `TODO无此联调字段_用途描述`、API mock、空 options、禁用态或可感知预留位显性暴露。需要可拓展结构时，可以定义前端本地 payload / type，但其中未确认字段仍必须用 TODO 属性名暴露，禁止用普通字段名加注释伪装成已确认契约。
...
15. **禁止为了完成目标伪造完整性**：没有接口、字段、枚举、图片、图表数据或数据源时，禁止为了让页面“看起来完整”而写静态默认值、示例数据、相似字段、本地计算或假兜底。可以保留可感知入口、空态、禁用态、TODO 占位或目标接口位置，等待后端或用户后续提供；不能做成用户误以为链路已完整的效果。
