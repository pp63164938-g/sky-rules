...
14. **禁止为了完成目标伪造完整性**：没有接口、字段、枚举、图片、图表数据或数据源时，禁止为了让页面“看起来完整”而写静态默认值、示例数据、相似字段、本地计算或假兜底。可以保留可感知入口、空态、禁用态、TODO 占位或目标接口位置，等待后端或用户后续提供；不能做成用户误以为链路已完整的效果。
15. **任何兜底逻辑必须交付告知**：本次新增、修改或保留的兜底逻辑，无论是展示占位、空数组、默认枚举、兼容旧字段、静态 Mock、异常返回还是接口缺字段处理，都必须在最终回复中列出位置、策略、依据、影响和后续处理。禁止只在代码注释或 diff 中体现。
