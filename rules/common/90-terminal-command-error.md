...
- ⚠️ 类型检查按打包门槛判断：
  - 先看 `package.json` 的 build / typecheck 脚本。脚本把 `vue-tsc`、`tsc` 放在产物构建前，类型失败会阻断打包时，本次改动交付前必须只跑该类型检查命令。
  - 命令用项目脚本里的类型检查部分，例如 `vue-tsc -b`。禁止改成文档里的另一种参数，也禁止顺手跑后面的 `vite build` / `webpack build`。
  - 类型检查失败必须告知用户，并区分本次引入和存量阻断。存量错误阻断打包时不能说验证通过。
  - 项目脚本没有把类型检查作为打包前置，或只有 IDE 插件检查时，仍按轻量验证；不把全项目类型扫描当成默认步骤。
  - 如果类型检查因项目存量问题、OOM 或无关文件报错失败，应停止继续加码重试，并明确说明失败原因和是否命中新改文件。
  - 禁止在未说明原因时自动提高 Node heap 继续反复跑全项目检查。

- ❌ 默认禁止：
  - 日常页面开发默认执行 `npm run build` / `pnpm build` / `yarn build`
  - 为了验证单个页面而触发全项目产物构建
  - 项目没有类型检查打包前置时，为了单个页面触发全项目类型扫描
