...
- 当前工作区不在本仓库时，优先读取 `~/.sky-rules/local.json` 的 `paths.skyRulesRepo` 定位源仓库；配置失效时询问用户，禁止扩大到磁盘扫描。
- 修改规则或工作流前必须先确认归属、查重、定位插入章节，禁止把新内容直接追加到文件末尾。
- 规则质量不得依赖用户在预览中发现问题；预览前必须完成不变量、约束对象、未来扩展、合法场景误伤、事实失效和触发可见性自审。
...
7. 同步验证：在仓库根目录执行 `python3 sync-workflows.py --no-git`；同步脚本默认会先执行 `python3 scripts/check-rules.py --source-only`，同步后再执行 `python3 scripts/check-rules.py`。
8. 输出回执：说明实际同步目标、拼接规则源、Codex 可见性、二次自检和提交状态。
...
- 核心原则：一句话说明规则要解决的问题。
- 适用场景：说明什么时候必须执行。
- 约束对象与边界：说明规则约束什么、不约束什么，以及当前事实变化后的处理方式。
- 硬性红线：列出禁止行为。
- 正反例：只保留最小抽象示例。
