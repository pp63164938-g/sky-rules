...
**核心原则**：用户调用本工作流但只说“加到规则里”，不代表内容只能进全局规则。先判断约束对象，再决定写规则、写工作流，或两边都写。

判断顺序：

1. 所有任务都要遵守的红线、禁止项、交付要求，写 `rules/`。
2. 只在某个工作流被调用时才执行的步骤、检查顺序、输入输出，写 `workflows/` 对应源文件。
...
预览和写入后的交付，都必须用独立清单列出每一处落点，不能只写在正文段落里。

- 只写全局规则：写「工作流 / Skill：无」。
- 写入工作流：逐条写源文件和同步后的 skill 名，例如 `workflows/base.xxx.md` → `base-xxx`。
- 两处都写：分成「全局规则」和「工作流 / Skill」两段，禁止合并成一句“已更新规则”。

**禁止行为**：

- 禁止因为用户说了“规则”，就把工作流步骤写进全局规则。
- 禁止因为用户没提 skill，就跳过工作流源文件检索。
- 禁止直接改同步后的 Skill 产物。工作流变更只改 `workflows/` 源文件。
- 禁止把工作流落点藏在段落、同步日志或“顺便更新”里。
