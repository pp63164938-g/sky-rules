...
3. **读取钉钉文档链接**
   ...
   - `a-doc-key` 必须使用文档 `docKey`，禁止把知识库节点 ID 或 `/i/nodes/` 路径段当作 `a-doc-key`。节点 ID 只能用于打开页面；正文接口 400 且错误码表示文档标识无效时，应改用用户 referrer / XHR 中的 `docKey` 或 `docId` 重试一次。
   - 钉钉图片资源 URL 可能先 302 到对象存储。下载时必须跟随跳转，并校验最终 HTTP 状态和文件字节数大于 0；禁止把 302 空响应写成下载成功。
   - 当前事实：正文接口为 `POST /api/document/data`，图片路径形如 `/core/api/resources/img/...`。若接口改为直出文件、不再 302，或请求头字段更名，本条操作细节失效，应回查实际 XHR 后更新本节，禁止在业务任务中静默兼容。
   - 资源文件落地不等于已视觉核验。本轮没有可用核验能力时，记录“已下载、未核验”及影响范围，并按全局「查证收口与回合可见性门禁」交卷；禁止继续空转或把下载成功说成已经看懂图片。
