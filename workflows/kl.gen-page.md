...
2. **防“四不像”高危雷区对照单**
   ...
   - 🔴 **雷区 5 补充：违反 Kunlun Element Plus Namespace**
     - ❌ 使用 `.el-*` 运行时类名或 `--el-*` CSS 变量默认前缀。
     - ❌ 同时保留 `.el-sky-*` 与 `.el-*`，为当前项目不会生成的前缀增加无依据兼容。
     - ✅ **项目约定：** Kunlun 的 Element Plus 运行时类名统一使用 `.el-sky-*`，CSS 变量统一使用 `--el-sky-*`；`<el-button>`、`<el-select>` 等 Vue 组件标签不受运行时 namespace 约束。
     - ✅ 页面、弹窗和组件生成必须遵循全局项目专用的“Element Plus Namespace 规范”，所有直接或间接引用运行时类名、CSS 变量或 DOM 结构的代码与配置均受约束。
     - ✅ 交付前必须定向检查本次修改文件，确保不存在默认运行时前缀或无依据双前缀兼容；存量错误进入本次修改范围时同步纠正。
   - 🔴 **雷区 6：临时权限标识随意发散**
